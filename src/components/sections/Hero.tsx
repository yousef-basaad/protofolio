...
export function Hero() {
  ...
  const heroWords = dict.hero.heroWords;
    ...
    <section id="top" className="relative flex min-h-[100svh] flex-col justify-center overflow-hidden pt-28 pb-16">
      ...
      <div className="container-x relative">
        ...
        <motion.div {...fadeUp(0.6)} className="mt-8 flex items-center gap-5 text-sm text-fg-muted">
          <SocialLink href={site.links.github} label={dict.contact.channelLabels.github} icon={<Github className="h-4 w-4" />} />
          <SocialLink href={site.links.linkedin} label={dict.contact.channelLabels.linkedin} icon={<Linkedin className="h-4 w-4" />} />
          <SocialLink href={`mailto:${site.email}`} label={dict.contact.channelLabels.email} icon={<Mail className="h-4 w-4" />} plain />
        </motion.div>
        ...
        >
          {dict.hero.stats.map((s, i) => (
            <div key={i} className="bg-bg px-5 py-4">
